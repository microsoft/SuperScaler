...
PollExecutor::PollExecutor(size_t max_worker_count) : m_max_worker_count(max_worker_count) {
}
        ...
        if (m_busy_workers.size() < m_max_worker_count) {
            ...
            auto itr = std::min_element(
                ...
                [](
                    ...
                    return real_w1->get_workload() < real_w1->get_workload();
            });
            worker = *itr;
...
void PollExecutor::release_worker(std::weak_ptr<Worker> worker) {
    ...
    real_worker->add_task(
        [&idle_workers, &busy_workers, &idle_mutex, worker] {
            auto real_worker = worker.lock();
            if (!real_worker) {
                return;
            }
