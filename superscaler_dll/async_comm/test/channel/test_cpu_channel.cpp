...
TEST(CPUChannel, SimpleSendReceive)
{
    ...
    bool result = false;
    result = channel.send("1", 1, 1, nullptr);
    ASSERT_TRUE(result);
    result = channel.send("2", 1, 1, nullptr);
    ASSERT_TRUE(result);
    result = channel.send("3", 1, 1, nullptr);
    ASSERT_TRUE(result);
    char output[3];
    result = channel.receive(output, 3, 1, nullptr);
...
TEST(CPUChannel, RecursiveSendRecv)
{
    ...
    result = channel.send("1", 1, 1, [&channel](bool, const void *, size_t) {
        bool cb_result = channel.send("2", 1, 1, nullptr);
    ...
    result = channel.receive(
        buffer, 1, 1, [&channel, &buffer](bool, void *, size_t) {
            bool cb_result = channel.receive(buffer + 1, 1, 1, nullptr);
            ASSERT_TRUE(cb_result);
        });
    ASSERT_TRUE(result);
...
TEST(CPUChannel, InvalidRank)
{
    ...
    CPUChannel channel({ 1 }, 100);
    bool result = true;
    result = channel.send("1", 1, invalied_rank, nullptr);
    ASSERT_FALSE(result);
    char buffer[20];
    result = channel.receive(buffer, sizeof(buffer), invalied_rank, nullptr);
    ASSERT_FALSE(result);
}
...
TEST(CPUChannel, ReceiveOutBound)
{
    ...
    std::thread th{ [&]() {
        char buffer[4];
        channel.receive(buffer, 4, 1, nullptr);
        {
...
TEST(CPUChannelDeathTest, InvalidSendBuffer)
{
    ...
    ASSERT_DEATH({ channel.send((void *)20, 1 << 20, 1, nullptr); }, "");
}
...
TEST(CPUChannelDeathTest, InvalidRecvBuffer)
{
    ...
    result = channel.send("1", 1, 1, nullptr);
    ASSERT_TRUE(result);
    ASSERT_DEATH({ channel.receive((void *)(20), 1, 1, nullptr); }, "");
}