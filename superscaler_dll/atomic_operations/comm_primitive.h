#ifndef COMM_PRIMITIVE_H_
#define COMM_PRIMITIVE_H_
...
public:
    ...
    ~RdmaCommPrimitive() {}
    void initialization(std::vector<std::string> ips, std::vector<std::string> ports, int myRank, int nRanks, int localRank, size_t size);
    float *send_receive(float *gradients, int sendTarget, int sendAddress, int sendLength, int receiveTarget, int receiveAddress, int receiveLength);
    void set_cfg_RDMA(int myRank, int nRanks, int localRank, size_t size);
