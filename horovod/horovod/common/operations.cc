...
#if HAVE_RDMA
#include "ops/super_operations.h"
...
#if HAVE_RDMA
RdmaCommPrimitive RdmaPrimitive;
#endif
...
OperationManager* CreateOperationManager(HorovodGlobalState& state) {
  ...
  allreduce_ops.push_back(std::shared_ptr<AllreduceOp>(
      new SuperAllreduce(&RdmaPrimitive, &mpi_context, &state)));
#endif
...
void BackgroundThreadLoop(HorovodGlobalState& state) {
...
#if HAVE_RDMA
  RdmaPrimitive.set_cfg_RDMA(horovod_global.controller->GetRank(),
                             horovod_global.controller->GetSize(),
                             horovod_global.controller->GetLocalRank(),
                             16*1024*1024);
#endif
  ...
  LOG(DEBUG, horovod_global.controller->GetRank())
