...
def get_common_options(build_ext):
    ...
    if compile_without_gloo:
        ...
        try:
            ...
            if compile_with_gloo:
                ...
                raise RuntimeError(
                    'Cannot find CMake. CMake is required to build Horovod with Gloo.')
        ...
        if is_mac:
            if compile_with_gloo:
                raise RuntimeError(
                    'Gloo cannot be compiled on MacOS. Unset HOROVOD_WITH_GLOO to use MPI.')
            print(
                'INFO: Gloo cannot be compiled on MacOS, will skip compiling Horovod with Gloo.')
    ...
    if not have_gloo and not have_mpi:
        raise RuntimeError(
            'One of Gloo or MPI are required for Horovod to run. Check the logs above for more info.')
    ...
    if cpu_operation:
        ...
        if cpu_operation.upper() == 'MPI':
            if not have_mpi:
                raise RuntimeError(
                    'MPI is not installed, try changing HOROVOD_CPU_OPERATIONS.')
            MACROS += [('HOROVOD_CPU_OPERATIONS_DEFAULT', "'P'")]
        elif cpu_operation.upper() == 'MLSL':
            if not have_mlsl:
                raise RuntimeError(
                    'MLSL is not installed, try changing HOROVOD_CPU_OPERATIONS.')
            MACROS += [('HOROVOD_CPU_OPERATIONS_DEFAULT', "'M'")]
        elif cpu_operation.upper() == 'GLOO':
            if compile_without_gloo:
                raise ValueError(
                    'Cannot set both HOROVOD_WITHOUT_GLOO and HOROVOD_CPU_OPERATIONS=GLOO.')
            if is_mac:
                raise RuntimeError(
                    'Cannot compile Gloo on MacOS, try changing HOROVOD_CPU_OPERATIONS.')
            elif not have_cmake:
                raise RuntimeError(
                    'Cannot compile Gloo without CMake, try installing CMake first.')
            MACROS += [('HOROVOD_CPU_OPERATIONS_DEFAULT', "'G'")]

    have_rdma = True

    if have_rdma:
        # Build RDMA Interface
        MACROS += [('HAVE_RDMA', '1')]
        INCLUDES += ['/var/drivers/nvidia/367.55/include']
        SOURCES += ['horovod/common/ops/super_operations.cc']
        LIBRARY_DIRS += ['/usr/lib/',
                         '/var/drivers/nvidia/367.55/lib64']
        LIBRARIES += ['rdmacm', 'ibverbs', 'pthread', 'dl']
        COMPILE_FLAGS += ['-libverbs', '-lrdmacm',
                          '-lcudart', '-lcuda', '-lpthread', '-ldl']
        LINK_FLAGS += ['-libverbs', '-lrdmacm',
                       '-lcudart', '-lcuda', '-lpthread', '-ldl']

        # Link lib_super_scaler
        INCLUDES += ['/usr/local/include/SuperScalerDll/']
        EXTRA_OBJECTS += ['/usr/local/include/SuperScalerDll/lib_super_scaler.a']
...
def find_matching_gcc_compiler_path(gxx_compiler_version):
    ...
    print('INFO: Unable to find gcc compiler (version %s).' %
...
def build_tf_extension(build_ext, global_options):
    ...
    if sys.platform.startswith('linux') and not os.getenv('CC') and not os.getenv('CXX'):
        ...
        cflags, cppflags, ldshared = remove_offensive_gcc_compiler_options(
            compiler_version)
    ...
    try:
        with env(CC=cc_compiler, CXX=cxx_compiler, CFLAGS=cflags, CPPFLAGS=cppflags,
            ...
            if options['BUILD_GLOO']:
                build_cmake(build_ext, gloo_lib, 'tf',
                            gloo_compile_macros, options, tensorflow_mpi_lib)
...
def build_torch_extension_v2(build_ext, global_options, torch_version):
    ...
    gloo_abi_flag = ['-D_GLIBCXX_USE_CXX11_ABI=' +
                     str(int(torch.compiled_with_cxx11_abi()))]
    ...
    try:
        with env(CC=cc_compiler, CXX=cxx_compiler, CFLAGS=cflags, CPPFLAGS=cppflags,
                 LDSHARED=ldshared):
            if options['BUILD_GLOO']:
                build_cmake(build_ext, gloo_lib, 'torchv2',
                            gloo_abi_flag, options, torch_mpi_lib_v2)
            customize_compiler(build_ext.compiler)
...
def build_cmake(build_ext, ext, prefix, additional_flags, options, plugin_ext=None):
    ...
    lib_output_dir = os.path.abspath(
        os.path.join(build_ext.build_temp, 'lib', prefix))
    ...
    build_temp = os.path.abspath(os.path.join(
        build_ext.build_temp, ext.name, prefix))
