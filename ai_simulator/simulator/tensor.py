...
class Tensor():
    ...
    valid_type = {'float':4,'double':8,'char':1,'int8':1,'int16':2,'int32':4,'int64':8}
    def __init__(self, 
                 tensor_type='int8',
                 tensor_size=0):
        ...
        # Check type
        if not self.type in Tensor.valid_type:
            raise TensorException(
                '[ERROR] Tensor initialization failure because unsupported data_type: %s' % self.type)
        # Int, size * sizeof(type) = data_total_bytes
        self.size = tensor_size
        # Check size
        if self.size<0:
            raise TensorException(
                '[ERROR] Tensor initialization failure because data size is negative: %s' % self.size)
        if not isinstance(self.size,int):
            raise TensorException(
                '[ERROR] Tensor initialization failure because data size is not an integer: %s' % self.size)        
