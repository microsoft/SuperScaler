...
def simulate_single_model(model_file_path):
    ...
    if not model_file_path.endswith(".pbtxt"):
        logger.error('Input model file %s can not be parsed!' %
                     model_file_path)
        return
    test_tf_adapter = TFAdapter()
    # Parse the tensorflow graph
    test_graph = test_tf_adapter.parse_protobuf_graph(
        test_tf_adapter.load_pbtxt_file(model_file_path))
...
def main():
    ...
    parser.add_argument(
        "-g", "--graph",
        help="The path to the model (either a directory or a file)",
