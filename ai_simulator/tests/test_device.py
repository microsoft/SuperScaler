...
def test_device_module():
	...
	assert test_device.is_idle() == True
	...
	assert test_FIFO_device.is_idle() == False
	assert test_FIFO_device.name() == "GPU:0"

	# Test head_node()
	actual_next_node , actual_next_finish_time = test_FIFO_device.get_next_node()
	assert actual_next_node == test_enqueue_node
	assert actual_next_finish_time == 3.0

	# Test dequeue_node()
	test_FIFO_device.dequeue_node()
	assert test_FIFO_device.is_idle() == True
